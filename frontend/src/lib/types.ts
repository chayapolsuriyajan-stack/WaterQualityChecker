...
/**
 * `GET /ai-report`'s success shape (main.py's `_generate_ai_report`/daily scheduler) -- a
 * plain-language daily summary of a station's water-quality stats, generated by Gemini.
 * `date`/`report` are both null when the station has no report yet (fresh deployment, or
 * before the first midnight/manual generation).
 */
...
/** `POST /ai-report/generate`'s success shape -- `cached: true` means the backend's
 * per-station cooldown (main.py's AI_REPORT_COOLDOWN_SECONDS) was still active, so no new
 * Gemini call was made; the already-stored report was returned as-is. Every admin who
 * clicks "Generate now" within the cooldown window gets the exact same text, and only the
 * first click actually spent a request -- see _generate_ai_report's header comment. */
export interface GenerateAiReportResponse extends AiReportResponse {
  cached: boolean
}
