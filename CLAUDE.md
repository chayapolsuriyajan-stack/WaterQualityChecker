...
**`main.py`** (FastAPI) is the one and only backend — referenced by `webconfig.json` and the firmware's `backendPort`. Handles HTTP sensor ingestion, WebSocket UI broadcast, local SQLite persistence (`storage.py`), and UDP backend discovery. (`server.js`, an earlier Node `ws` relay, no longer exists; `ws_client.py` is a standalone WebSocket test client, not a second backend.)
...
- **Config**: `webconfig.json` sets `staticDir` (WebGL build folder), `googleSheetsWebhookUrl`, `calibrationFile`, `historyDbFile` (local SQLite; `""` disables it, default `history.db`), `historyRetentionDays` (pruned once at startup; `0` = keep everything, default 365), `vapidPrivateKeyFile`/`vapidPublicKey`/`vapidSubject` + `httpsCertFile`/`httpsKeyFile`/`httpsPort` (Push notifications below), and `updateApiKey` (`/update` auth, below). Missing file/keys fall back to defaults.
- **Sensor ingestion** — `POST /update`: if `updateApiKey` is set, requires a matching `X-API-Key` header (constant-time), 401 otherwise; empty/missing (default) leaves it unauthenticated — matters once the fixed-backend-host override (WiFi provisioning below) makes it internet-reachable. Accepts JSON (`{temperature, turbidity, tdsVoltage, flowPulses}`, or legacy `{temperature, turbidity, tds}`) or form-urlencoded with `water_level`. Normalizes into a payload, derives `ec` from `tds` (`ppm_to_ec` — same DFRobot measurement pre the EC→TDS ×0.5 conversion, so it always agrees with TDS), appends to `history_buffer`, broadcasts to `/ws/app` clients as `sensor_update`. See Sensor calibration below, Flow sensor below for `flowPulses`.
...
- **Calibration mode gate**: saved coefficients apply only when `calibration_mode` is ON (`/calibrate` page's toggle, `POST /calibration/mode`). OFF → `/update` always emits raw ADC (`turbidityUnit: "ADC"`) and uncalibrated ppm (k=1.0) — a saved-but-unverified calibration can't silently affect live dashboards until switched on.
- **`/update` output**: emits both raw and calibrated fields. `turbidity` carries **calibrated NTU** once a calibration exists, else falls back to raw ADC (why this field shape: see `AQUA_MONITOR_PLAN.md`'s Appendix). `turbidityRaw` always carries raw ADC (calibration page + Sheets logging), `turbidityUnit` is `"NTU"`/`"ADC"`, `turbidityNtu` is calibrated NTU or `null`. TDS: `tdsVoltage` (raw) and `tds` (calibrated ppm).
- **Calibration page** — `GET /calibrate` serves `web/calibrate.html`, standalone (independent of the frontend's Calibration tab) — polls `GET /calibration` ~1s for live raw values, lets you enter a reference + capture a point (averaging the last ~5 raw readings), shows computed coefficients, saves. Tracks an observed min/max of the live raw reading (client-side) with "use min"/"use max" buttons; "Reset range" clears only that local tracking, never the saved calibration.
...
- **Water usage** (`payload["waterUsageToday"]`, liters) — a **daily-resetting** counter in `storage.py`'s `daily_usage(date, total_liters)`, keyed by local date. A day rollover starts a fresh row automatically; `POST /flow/reset-today` zeroes it manually. `GET /flow/usage?days=N` returns today's total plus the last N days', for the "Water Usage" bar chart — separate from `/history`'s merge (different shape/cadence: one row/day, not subject to `historyRetentionDays`).
...
- **`doPost` inserts each new reading at row 2** (right after the header) so the newest reading is always the top data row — older rows from before this change just sit further down, untouched (migration story: `AQUA_MONITOR_PLAN.md`'s Appendix). Columns: `Timestamp, Temperature (C), Turbidity (raw ADC), TDS (ppm), Flow Rate (L/min)`.
- `doGet` accepts `?seconds=`/`?maxPoints=` (default 900s/400), reads a **leading** slice (rows 2..N, newest first — matching the insert-at-top write above), reverses to chronological order, filters to the window, and stride-downsamples to `maxPoints` so long windows stay fast.
- **`GET /history?window=`** — selectable window (`5m`/`15m`/`1h`/`3h`/`12h`/`24h`, default `15m`, `HISTORY_WINDOWS`). **Three tiers, cheapest first**, merged into one chronological list; the response's `source` field reports which contributed (e.g. `db`, `db+sheet`, `live`):
  ...
  3. **Google Sheets** — proxies `doGet` with matching `seconds`/`maxPoints` (same-origin, avoiding CORS/redirect issues with Google), consulted *only* for the part of the window local sources don't reach: a fresh DB, a restored machine, or readings predating the DB.

  The gap test allows `HISTORY_GAP_TOLERANCE_MS` (15s) slack — readings arrive every 2s, so the first row at/after a cutoff is always a second late, and an exact-match test made every steady-state request look like a gap, firing pointless Sheets round-trips. Every row gets `turbidityNtu` (via `_with_ntu`) and `ec` (from `tds` if absent) added uniformly across all three sources.
- **Local history database** (`storage.py`, `historyDbFile`) — every `/update` reading is written to SQLite (WAL, `synchronous=NORMAL`) via `asyncio.to_thread`, fire-and-forget, so disk latency never delays the response. Column names mirror `history_buffer` and `doGet` (`timestamp` epoch **milliseconds**, `turbidity` = raw ADC) so all three sources merge without translation. Failures here are never fatal — a bad path/permission error just logs and degrades to buffer + Sheets. Git-ignored (measurement data, not code); back it up separately.
...
- **`webSerial.ts` (browser-side, current)**: mirrors `wifi_serial.py`'s API and wire protocol (`WIFI_SCAN`/`WIFI_SET`/`WIFI_STATUS`, `BACKEND_SET`/`BACKEND_CLEAR`/`BACKEND_STATUS`/`BACKEND_TEST`). One module-level `SerialPort` from `navigator.serial.requestPort()`, read via a `TextDecoderStream`-piped loop into a line buffer (Web Serial has no line framing). `connect()` pays the same ~2s DTR/RTS settle delay. A `disconnect` listener resets `WifiPanel.tsx`'s state, clears `wifi-status`/`wifi-backend` query caches.
...
- Two earlier dashboards (`web-react/`, a prebuilt SPA at `/`; and a hand-built `web/index.html`+`app.js`+`style.css` at `/classic`) were **removed** — neither files nor routes exist any more; see `AQUA_MONITOR_PLAN.md`'s Appendix for why. **Do not add code paths or comments referring to either.** `web/calibrate.html` (Sensor calibration above) is the one survivor from `web/`.
...
Reads DS18B20 temperature, analog turbidity, TDS, and flow-sensor pulses every 2s and POSTs JSON `{temperature, turbidity, tdsVoltage, flowPulses}` to `http://<backendIP>:8080/update` — **all raw**: `turbidity` = averaged ADC, `tdsVoltage` = raw voltage, `flowPulses` = pulses over that interval. TDS formula, temp compensation, NTU conversion, and flow k-factor live on the **backend** (Sensor calibration below), recalibratable without reflashing. (Legacy firmware POSTing pre-computed `tds`, or omitting `flowPulses`, still works.) Backend IP isn't hardcoded: `discoverBackend()` broadcasts `HYDRO_DISCOVER` over UDP:8888, taking the replying host's IP. Discovery runs once at boot (blocking, **bounded to 20s**) and again after 3 consecutive `/update` failures (`consecutiveFailures`/`backendKnown`); `loop()` keeps retrying on its own timer until then.

**Google Sheets fallback (no backend needed)**: when unreachable but Wi-Fi/internet still work, readings go into a 30-entry circular buffer (`sheetsFallbackBufferPush`, ~60s at 2s cadence), flushed once per `sheetsFallbackInterval` (60s) by POSTing each reading **individually to the same Apps Script Web App** the Sheets relay uses (`doPost` doesn't distinguish the paths). Overflow drops the oldest reading; buffer clears after a flush and again once the backend POST succeeds. Requires `sheetsWebhookUrl` to match `googleSheetsWebhookUrl`. **Calibration caveat**: `turbidity` is raw ADC, recalibrating retroactively via `_with_ntu`; `tds` can't — no raw-voltage column to backfill — so firmware sends **uncalibrated** ppm (`dfrobotUncalibratedPpm`, k=1.0), never the personal k-factor. `flowPulses` is **not** included — only temperature/turbidity/TDS.
