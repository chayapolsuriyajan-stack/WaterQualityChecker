...
AquaMonitor: a water quality monitoring system. A single ESP32 board reads DS18B20 temperature, analog turbidity, TDS, and flow-sensor pulses, and POSTs readings to a backend, which relays them to a live browser dashboard and, on breaches, to subscribed browsers as OS push notifications (Push notifications below). No camera/image-based inference (an earlier ESP32-CAM + Roboflow YOLO pipeline was removed).

**`main.py`** (FastAPI) is the one and only backend — referenced by `webconfig.json` and the firmware's `backendPort`. Handles HTTP sensor ingestion, polled UI live-state (`GET /live`), Turso persistence for push subscriptions + daily water usage (`storage.py`), and a legacy UDP backend-discovery listener (kept but unused by this repo's own firmware — see Backend discovery below). (`server.js`, an earlier Node `ws` relay, no longer exists; `ws_client.py` is a standalone WebSocket test client, not a second backend — and no longer reflects `main.py`'s live-data path now that `/ws/app` is gone.)

Firmware source lives at `firmware/AquaMonitor/AquaMonitor.ino` (moved here from the standalone Arduino sketches folder). Open the `.ino` directly in the Arduino IDE — the containing folder name matches the sketch name, which the IDE requires.
...
- **Backend discovery** — a UDP listener (`DiscoveryProtocol`, started from the `lifespan` context manager) binds `0.0.0.0:8888`, replies `HYDRO_HERE` to `HYDRO_DISCOVER`, so LAN firmware *could* find this machine's IP without hardcoding — kept for local deployments/third-party tools, though this repo's own ESP32 firmware no longer uses it (see Firmware below: manual `BACKEND_SET` over USB is the only path this board uses to learn a backend host). Requires a Windows Firewall inbound-UDP-8888 rule (`netsh advfirewall firewall add rule name="AquaMonitor UDP Discovery" dir=in action=allow protocol=UDP localport=8888` — a fresh machine lacks this and discovery silently times out until added). Only finds a backend on the same subnet as whatever client emits `HYDRO_DISCOVER` (UDP broadcast doesn't cross networks).
...
- **Auth stays shared**: `updateApiKey` is not per-station. UDP discovery is not part of any
  board's connection path any more (see Firmware/WiFi provisioning below — manual `BACKEND_SET`
  is the actual mechanism), so multi-station support has nothing to say about it either way.
  **Push subscriptions** (`storage.py`'s `push_subscriptions`, keyed by browser endpoint) stay
  as-is — inherently per-browser-dashboard, not per-station; only the breach edge-detection
  (`last_severity`) is keyed by station.
...
- **HTTPS listener**: Web Push needs a secure context — `http://localhost` is exempt only on the *same machine*, so a phone/other LAN device needs real HTTPS. `webconfig.json`'s `httpsCertFile`/`httpsKeyFile` (empty by default — off) and `httpsPort` (default 8443) start a **second** uvicorn `Server` alongside the always-on HTTP:8080 one, both serving the same FastAPI `app`. `lifespan` runs once per `uvicorn.Server` instance, so a `_discovery_listener_started` guard stops the UDP listener rebinding port 8888 (would crash the second server). A self-signed cert works for LAN testing but needs its CA trusted per-device (Android refuses an untrusted cert even after "proceed anyway").
- **Subscription storage** (`storage.py`'s `push_subscriptions`) — one row per browser push endpoint (no user/account concept in this app), holding endpoint URL, `p256dh`/`auth` keys, a `prefs_json` blob (`{param: {warn: bool, danger: bool}}`, default warn=off/danger=on for `temperature`/`turbidity`/`tds`/`ec` — **not** `flow`, no thresholds), and a `lang` column (`"en"`/`"th"`, default `"en"` — Notification language below). `lang` was added via `_migrate_push_subscriptions_lang`'s plain `ALTER TABLE ADD COLUMN` (no primary-key change needed, unlike `daily_usage`'s rebuild) — existing rows default to `"en"`, matching their behavior before the column existed.
- **Notification language**: every push a subscription receives — breach alert, test — is in whichever language its `lang` column holds, picked from `PARAM_DISPLAY`/`PARAM_DISPLAY_TH` + `SEVERITY_TH` (`_format_push_text`) and `_PUSH_ACTIONS` (action-button labels). Set at subscribe time from the frontend's current UI language (`subscribeToPush(lang)`, `frontend/src/lib/i18n.tsx`'s `useT().lang`) and kept in sync afterward by `PushLangSync` (`App.tsx`) — a tiny always-mounted component that watches `useT().lang` and calls `syncPushLang` (`push.ts`) whenever it changes, so a subscriber who switches languages later doesn't keep getting alerts in their old one until they happen to reopen Settings. `PUT /push/preferences` accepts an optional `lang` for this re-sync, separate from the initial `POST /push/subscribe` one.
- **Trigger**: `_check_breaches_and_dispatch` runs inline on every `/update` (must see every reading to edge-detect), comparing status against `last_severity` — fires only on the **good→warn/danger transition** (re-arms on recovery). Sends are deferred via `asyncio.create_task` off Vercel, same pattern as the Sheets relay, so a slow push service never delays the ESP32's response — but awaited directly on Vercel (`IS_VERCEL`, below), since a fire-and-forget task risks being silently dropped when the serverless container freezes right after the response.
- **Payload**: `PARAM_DISPLAY`/`PARAM_DISPLAY_TH` (emoji/label/unit, hand-mirrored from `paramMeta.ts` like `thresholds.py` mirrors `RANGE_BANDS`) builds a title/body, an `icon`/`badge` (app favicon), two action buttons (`view` opens the dashboard, `dismiss` closes) — consumed by the service worker below. A dead subscription (404/410) self-heals by deleting that row.
- **REST API**: `GET /push/vapid-public-key`, `POST /push/subscribe` `{endpoint, keys:{p256dh,auth}, prefs?, lang?}`, `POST /push/unsubscribe` `{endpoint}`, `GET /push/preferences?endpoint=` (returns `{prefs, lang}`), `PUT /push/preferences` `{endpoint, prefs, lang?}`, `POST /push/test` `{endpoint}` (bypasses prefs/thresholds, backs Settings' test-notification button, sent in the subscription's stored `lang`). All 503 if Turso storage is disabled.
- **Frontend**: `frontend/src/lib/push.ts` is the browser-side plumbing (Service Worker registration, subscribe/unsubscribe, prefs get/save, `syncPushLang`, `sendTestPush`); `frontend/public/sw.js` is **plain vanilla JS** (not Vite-built — no imports/TypeScript) handling `push`/`notificationclick`. The UI lives in the **Settings** dialog (Frontend below), not a standalone bell icon.
...
- **Trigger**: an `asyncio` background task (`_daily_report_scheduler`, started once from
  the `lifespan` context manager behind a `_daily_scheduler_started` guard — same double-startup
...
- **`IS_VERCEL`** (`bool(os.getenv("VERCEL"))`, set automatically by every Vercel
  build/runtime) gates two pieces of startup behavior that only make sense for a
  ...
  see AI daily report above). It also gates two request-path fire-and-forget-vs-awaited
  choices inside `POST /update` (the Google Sheets relay, Push notifications' breach
  dispatch): fire-and-forget off Vercel (don't block the ESP32's response), awaited directly
  on Vercel, since a task merely created but not yet awaited can be silently dropped when
  the serverless container freezes right after the HTTP response is sent.
...
- **Fixed backend host override**: `BACKEND_SET` is the only way any board learns a backend host — same-network or not. There is no discovery fallback on this firmware (an earlier `discoverBackend()` UDP broadcast was removed); every board requires an explicit host configured over USB before it can POST anywhere.
...
- **Stack**: Vite + React 19 + TypeScript + Tailwind v4, shadcn/ui + Recharts + Motion + TanStack Query. `npm run build` outputs `frontend/dist/` (git-ignored, along with `node_modules/`); `/` 404s until built at least once. `npm run dev` proxies `/live`, `/history`, `/calibration*`, `/update`, `/push*`, `/flow*`, `/wifi*`, `/ai-report*`, `/station*` to `:8080` for HMR against a live `python main.py`.
- **Same-origin only, by design**: every frontend fetch (`api.ts`'s shared `request()` helper, plus `push.ts`'s own inline calls) uses a bare relative path — no hardcoded host, no absolute URL, anywhere in the frontend. This is the only deployment shape this app supports: frontend and backend are always served from the same origin, whether that's `main.py`'s own `StaticFiles` mount (local/Windows-service) or Vercel's `rewrites` + static-output combination (Vercel deployment below). Hosting the dashboard on one domain while `main.py` runs reachable only at a different one is explicitly **not** supported — no CORS middleware exists or is planned, and no configurable API base URL exists; adding either would be real surface area for a deployment shape nobody uses.
...
## Firmware (`firmware/AquaMonitor/AquaMonitor.ino`)
