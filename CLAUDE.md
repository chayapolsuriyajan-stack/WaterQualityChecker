...
python main.py
```
Serves on `0.0.0.0:8080` with `reload=True`. The default page (`frontend/`, see Frontend below) must be built first: `cd frontend && npm install && npm run build`, which produces `frontend/dist/`, mounted at `/`. A WebGL build directory (default `Build/`, configurable via `webconfig.json`'s `staticDir`) is mounted at `/{staticDir}` with Brotli/gzip content-encoding support for precompressed assets — unrelated legacy Unity output, not part of the dashboard.
...
- **Stack**: Vite + React 19 + TypeScript + Tailwind v4, shadcn/ui + Recharts + Motion + TanStack Query. Build with `cd frontend && npm install && npm run build` (outputs `frontend/dist/`, git-ignored along with `node_modules/`); `/` 404s until this has been built at least once. `npm run dev` runs a Vite dev server that proxies `/ws/app`, `/history`, `/calibration*`, `/update` to `:8080` for HMR against a live `python main.py`.
- **Left sidebar shell** with three tabs — **Dashboard** (WQI history chart with a time-range selector and reference lines; a 2×2 live param grid for Temperature/Turbidity/TDS/EC with 30s sparklines; clicking any card opens a detail modal with a bigger chart, min/avg/max, and a two-sided too-high/too-low warning; 3 radial safety gauges), **Calibration** (turbidity 2-point + TDS k-factor wired to the real `/calibration*` API, optimistic apply + toast, observed-range min/max/reset — see Sensor calibration above), **History** (`/history` table + CSV export) — plus a light/dark theme toggle and an EN/ไทย language switcher (all UI text centralized in `frontend/src/lib/strings.ts`).
...
- Previous dashboards — a black-box prebuilt React SPA (`web-react/`, generated externally, could not be edited) at `/` and a hand-built vanilla dashboard (`web/index.html`+`app.js`+`style.css`) at `/classic` — have been **removed**. `/classic` no longer exists. `web/calibrate.html` (see Sensor calibration above) is the one survivor from `web/`, kept as a lightweight standalone alternative to the main app's Calibration tab.
