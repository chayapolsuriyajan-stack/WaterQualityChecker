...
**`main.py`** (FastAPI) is the one and only backend — referenced by `webconfig.json` and the firmware's `backendPort`. Handles HTTP sensor ingestion, polled UI live-state (`GET /live`), Turso persistence for push subscriptions + daily water usage (`storage.py`), and UDP backend discovery. (`server.js`, an earlier Node `ws` relay, no longer exists; `ws_client.py` is a standalone WebSocket test client, not a second backend — and no longer reflects `main.py`'s live-data path now that `/ws/app` is gone.)
