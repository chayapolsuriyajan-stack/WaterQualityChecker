...
- `doGet` serves the last up to 500 rows back as JSON. `GET /history` in `main.py` proxies this server-side (keeping the dashboard's fetch same-origin, avoiding browser CORS/redirect issues with Google) and filters to the last 15 minutes (`HISTORY_WINDOW_SECONDS`) for the dashboard's history graph.
